...
void solve(vector<pair<int, vector<int> > > &v) {
	...
	for(auto t:v) {
		bool poss = true;
		for(int i = 0; i < 10; i++) {
			if((i == 1 or i == 7) and t.second[i] >= 0 and t.second[i] <= arr[i]) {
				poss = false;
				break;	
			} else if((i == 3 or i == 6 ) and t.second[i] >= 0 and t.second[i] >= arr[i]) {
				poss = false;
				break;
			}
			else if((i != 3 and i != 6 and i != 1 and i != 7) and t.second[i] >= 0 and t.second[i] != arr[i]) {
