...
int mem[4][4][150][150];
pair<pair<int, int>, pair<int, int> > par[4][4][150][150];
bool bio[150][150];
...
void solve(vector<string> &v) {
	...
	s.insert({0, {{0, 0}, {0, 0}}});
	const int INF = 1e9;
	for(int i  = 0; i < 4; i++) for(int j = 0; j < 4; j++) for(int k = 0; k < v.size(); k++) for(int l = 0; l < v[0].size(); l++) mem[i][j][k][l] = INF;
	mem[0][0][0][0] = 0;
	while(s.size()) {
		auto x = s.begin()->second;
		s.erase(s.begin());
		//cout << x.f.f <<" " << x.f.s << " " << x.s.f << " " <<x.s.s << '\n';
		if(x.s.f == v.size() - 1 and x.s.s == v[0].size() - 1) {
			s.clear();
		...
		for(int i = 0; i < 4; i++) {
			...
			if(xx < 0 or xx >= v.size() or yy < 0 or yy >= v[0].size() or zz > 3 or (ll - x.f.f + 4)%4 == 2) continue;
